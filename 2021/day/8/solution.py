...
def part_2(test=False) -> None:
    ...
    for entry in solution.entries:
        ...
        digit_dict[6] = set(
            next(
                filter(
                    lambda pattern: len(pattern) == 6
                    and not digit_dict[1].issubset(pattern),
                    entry[0],
                )
            )
        )
        ...
        digit_dict[5] = set(
            next(
                filter(
                    ...
                    and not segment_dict["top_right"] in pattern,
                    entry[0],
                )
            )
        )
        ...
        digit_dict[9] = set(
            next(
                filter(
                    lambda pattern: len(pattern) == 6
                    and not segment_dict["bottom_left"] in pattern,
                    entry[0],
                )
            )
        )
        ...
        digit_dict[0] = set(
            next(
                filter(
                    lambda pattern: len(pattern) == 6
                    and digit_dict[1].issubset(pattern)
                    and segment_dict["bottom_left"] in pattern,
                    entry[0],
                )
            )
        )
