...
class Solution:
    ...
    def __init__(self, input_file_path, test=False) -> None:
        input_file = open(f'{input_file_path}{"test.txt" if test else "input.txt"}', encoding="utf-8")
        ...
        self.median_total_fuel_used = int(sum(abs(self.median_alignment_position - position) for position in self.crab_positions))

        # mean
        self.mean_alignment_position = round(mean(self.crab_positions)) - 1
        self.mean_distances = [abs(self.mean_alignment_position - position) for position in self.crab_positions]
        self.mean_total_fuel_used = int(sum((distance ** 2 + distance)/(2) for distance in self.mean_distances))
        input_file.close()
